								...
								<div class="container-fluid">
									<div class="d-flex align-items-center justify-content-between">
										...
										<div class="header-buttons-main mt-0">
											<a class="btn btn-outline-light header-buttons text-center" href="{{url('/admin/createticket')}}"><i class="fa fa-paper-plane-o pe-lg-2 d-inline-block"></i><span class="d-m-none">{{lang('Create Ticket', 'Menu')}}</span></a>
											<a class="btn btn-outline-light header-buttons text-center visitsite ms-2" href="{{route('home')}}" target="_blank"><i class="fe fe-home pe-lg-2 d-inline-block"></i><span class="d-m-none">{{lang('Home Page', 'Menu')}}</span></a>
												...
												<a href="#" class="nav-link pe-1 ps-0 leading-none" data-bs-toggle="dropdown">
													<span>
														...
														@else
															...
															<img src="{{ route('getprofile.url', ['imagePath' =>Auth::user()->image,'storage_disk'=>Auth::user()->storage_disk ?? 'public']) }}" class="avatar avatar-md bradius rounded-circle " alt="{{Auth::user()->image}}">
															{{-- <img src="@if(Str::contains(Auth::user()->image, "storj")) {{ route('profile.url', ['imagePath' =>Auth::user()->image]) }} @else {{asset('uploads/profile/'.Auth::user()->image)}} @endif" class="avatar avatar-md bradius rounded-circle " alt="{{Auth::user()->image}}"> --}}
														@endif
