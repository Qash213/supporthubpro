										...
										<div class="dropdown profile-dropdown">
											<a href="#" class="nav-link pe-1 ps-0 leading-none" data-bs-toggle="dropdown">
												<span>
													...
													<img src="{{ route('getprofile.url', ['imagePath' =>Auth::guard('customer')->user()->image,'storage_disk'=>Auth::guard('customer')->user()->storage_disk ?? 'public']) }}" alt="{{Auth::guard('customer')->user()->image}}" class="avatar avatar-md bradius">
													{{-- <img src="@if(Str::contains(Auth::user()->image, "storj")) {{ route('profile.url', ['imagePath' =>Auth::user()->image]) }} @else {{asset('uploads/profile/'.Auth::user()->image)}} @endif" alt="{{Auth::guard('customer')->user()->image}}" class="avatar avatar-md bradius"> --}}
													@endif
