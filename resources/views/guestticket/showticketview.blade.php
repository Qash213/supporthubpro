...
@extends('layouts.usermaster')
											...
											<table class="table mb-0 table-information">
												<tbody>
													...
													<tr>
														...
														<td>:</td>
														<td>
															<span class="font-weight-semibold">{{ \Carbon\Carbon::parse($ticket->created_at)->format(setting('date_format'))}} </span>
														</td>
													</tr>
													<tr>
														<td>
						...
						<div class="col-xxl-9">
							<div class="card">
								<div class="card-header border-0 mb-1 d-block">
									...
									<small class="fs-13 "><i class="feather feather-clock text-muted me-1"></i>{{lang('Created At')}} <span class="text-muted">{{\Carbon\Carbon::parse($ticket->created_at)->diffForHumans()}}</span></small>
								</div>
                            ...
                            @if($holidays->isNotEmpty() && setting('24hoursbusinessswitch') != 'on')
                                @foreach ($holidays as $anct)
                                    <div class="alert alert-holiday p-5" role="alert">
                                        <button type="submit" class="btn-close ms-5 float-end text-danger notifyclose" data-id="{{$anct->id}}">×</button>
                                        <div class="d-flex align-items-start gap-3">
                                            ...
                                            <div>
                                                <h5 class="mb-1 holiday-title">{{$anct->occasion}}</h5>
								...
								@if (setting('USER_REOPEN_ISSUE') == 'yes' && $ticket->ticketreopen == null)
								    @if ($ticket->tickettype != 'emalitoticket')
    									@if (setting('USER_REOPEN_TIME') == '0')
        									<div class="card">
        										<form method="POST" action="{{url('guest/closed/' .encrypt($ticket->ticket_id))}}">
        											@csrf
        											@honeypot
                                                    ...
                                                    <div id="collapseFour" class="panel-collapse collapse" role="tabpanel" aria-expanded="false">
                                                        <div class="panel-body p-0">
                                                            <div class="card border-0 shadow-none mb-0">
                                                                <form method="POST" action="{{url('guest/ticket/'. encrypt($ticket->ticket_id))}}" enctype="multipart/form-data">
                                                                    @csrf
                                                                    @honeypot
                                                                    <input type="hidden" name="ticket_id" value="{{ $ticket->id }}">
                                                                    <div class="card-body">
                                                                    ...
                                                                    <div class="card-footer">
                                                                        <div class="form-group float-end">
                                                                            <button type="submit" class="btn btn-secondary" onclick="this.disabled=true;this.innerHTML=`Re-Opening <i class='fa fa-spinner fa-spin'></i>`;this.form.submit();">{{lang('Reply Ticket')}}</button>
                                                                        </div>
                                                                    </div>
                                                                </form>
                                                            </div>
		...
		<script src="{{asset('build/assets/plugins/dropzone/min/dropzone.min.js')}}?v=<?php echo time(); ?>"></script>
