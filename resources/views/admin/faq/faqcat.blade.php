...
<div class="col-xl-12 col-lg-12 col-md-12">
    <div class="card ">
        <div class="card-header border-0">
			<h4 class="card-title">{{lang('Create FAQ’s', 'menu')}}</h4>
		</div>
        <form action="{{route('faq.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('question') is-invalid @enderror" placeholder="{{lang('FAQ Question')}}" name="question" id="question"  autofocus required>
                    @error('question')
                ...
                <div class="form-group">
                    ...
                    <textarea class="summernote d-none @error('answer') is-invalid @enderror" placeholder="{{lang('FAQ Answer')}}" name="answer" id="answer" aria-multiline="true"></textarea>
                    @error('answer')
                        <span class="invalid-feedback d-block" role="alert">
                            <strong>{{ lang($message) }}</strong>
                        </span>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2-show-search  select2 @error('faqcatsname') is-invalid @enderror" data-placeholder="{{lang('Select Faq Category')}}" name="faqcatsname" id="faqcatsname">
                        ...
                        @foreach($faqcategorys as $faqcategory)
                            <option></option>
                            <option value="{{$faqcategory->id}}">{{$faqcategory->faqcategoryname}}</option>
                        @endforeach
                    </select>
                    @error('faqcatsname')
                        <span class="invalid-feedback d-block" role="alert">
