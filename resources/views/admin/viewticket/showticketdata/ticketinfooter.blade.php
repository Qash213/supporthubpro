<div class="card">
		...
		<div class="table-responsive tr-lastchild">
			<table class="table mb-0 table-information">
				<tbody>
					...
					@if ($ticket->subcategory != null && $ticket->subcategoriess)
						<tr>
							<td>
					...
					@if($ticket->ticketviolation == 'on')
                    <tr>
                        <td>
                            <span class="w-50">{{lang('Ticket Violation')}}</span>
                        </td>
                        <td>:</td>
                        <td>
	...
	<div class="card-footer ticket-buttons">
		@if($ticket->status == 'Closed')
			...
			<button class="btn btn-secondary my-1" id="reopen" data-id="{{encrypt($ticket->id)}}"> <i class="feather feather-rotate-ccw"></i> {{lang('Re-Open')}}</button>
            ...
            @if($ticket->status == 'Suspend' || $ticket->status == 'Closed')
                <div class="btn-group">
                    @if($ticket->ticketassignmutliples->isNotEmpty() && $ticket->selfassignuser_id == null)
                        @if ($ticket->ticketassignmutliples->count() == 1)
                            @php
                                $assigneddata = $ticket->ticketassignmutliples->first();
                                $assigneduserdata = \App\Models\User::find($assigneddata->toassignuser_id);
                            @endphp
                            <button class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown" disabled> {{ $assigneduserdata->name }} ({{ lang('Other Assign') }}) <span class="caret"></span></button>
                        @else
                        ...
                        @endif
                        <button data-id="{{encrypt($ticket->id)}}" class="btn btn-outline-primary" id="btnremove" disabled data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="{{lang('Unassign')}}" aria-label="Unassign"><i class="fe fe-x"></i></button>
                    @elseif($ticket->ticketassignmutliples->isEmpty() && $ticket->selfassignuser_id != null)

                    <button class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown"  disabled>{{$ticket->selfassign->name}} ({{lang('self')}}) <span class="caret"></span></button>
                    <button data-id="{{encrypt($ticket->id)}}" class="btn btn-outline-primary" id="btnremove" disabled data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="{{lang('Unassign')}}" aria-label="Unassign"><i class="fe fe-x"></i></button>
                    @else
            ...
            @else
                ...
                @else
                    <div class="btn-group">
                            ...
                            @else
                                @if ($ticket->ticketassignmutliples->count() == 1)
                                    @php
                                        ...
                                        $assigneduserdata = \App\Models\User::find($assigneddata->toassignuser_id);
                                    @endphp
                                    <button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown"> {{ $assigneduserdata->name }} ({{ lang('Other Assign') }}) <span class="caret"></span></button>
                                @else
                        ...
                        <button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown">{{lang('Assign')}} <span class="caret"></span></button>
                        @else
                        <button class="btn btn-outline-primary dropdown-toggle btn-sm" data-bs-toggle="dropdown">{{$ticket->selfassign->name}} ({{lang('self')}}) <span class="caret"></span></button>
                        <a href="javascript:void(0)" data-id="{{encrypt($ticket->id)}}" class="btn btn-outline-primary btn-sm" id="btnremove" data-bs-toggle="tooltip" data-bs-placement="top" title="" data-bs-original-title="{{lang('Unassign')}}" aria-label="Unassign"><i class="fe fe-x"></i></a>
                        @endif
                        @else
