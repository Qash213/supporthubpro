...
@section('content')
    ...
    <div class="page-header d-xl-flex d-block">
        <div class="page-leftheader">
            <h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{ lang('Ticket Information') }}</span>
            </h4>
        </div>
        <div class="page-rightheader ms-md-auto">
            <a href="javascript:void(0)" data-id="{{ encrypt($tickettrashedview->id) }}" class="btn btn-sm btn-danger"
                id="show-delete">
                <span> Delete Ticket </span>
                <i class="feather feather-trash-2 text-white" data-id="{{ encrypt($tickettrashedview->id) }}"
                    data-bs-toggle="tooltip" data-bs-placement="top" title="{{ lang('Delete') }}"></i>
            </a>
            <a href="javascript:void(0)" data-id="{{ encrypt($tickettrashedview->id) }}" class="btn btn-sm btn-info"
                id="show-restore">
                <span> Restore Ticket </span>
                <i class="feather feather-rotate-ccw text-white" data-id="{{ encrypt($tickettrashedview->id) }}"
                    data-bs-toggle="tooltip" data-bs-placement="top" title="{{ lang('Restore') }}"></i>
            </a>
        </div>
    </div>
    <!--End Page header-->
        ...
        <div class="col-xl-12 col-md-12 col-lg-12">
            <div class="row">
                <div class="col-xl-9 col-lg-12 col-md-12">
                    ...
                    @if ($tickettrashedview->purchasecode != null && decrypt($tickettrashedview->purchasecode) != "undefined")
                            ...
                            <div class="ps-0 pe-0 pb-0">
                                <div class="">
                                    ...
                                    @else
                                        ...
                                        @else
                                            <span
                                                class="">{{ Str::padLeft(Str::substr(decrypt($tickettrashedview->purchasecode), -4), Str::length(decrypt($tickettrashedview->purchasecode)), Str::padLeft('*', 1)) }}</span>
                                        @endif
                                    @endif
                                    <button class="btn btn-sm btn-dark leading-tight ms-2" id="purchasecodebtn"
                                        data-id="{{ $tickettrashedview->purchasecode }}">View Details</button>
                    ...
                    <div class="card">
                        <div class="card-header border-0 mb-1 d-block">
                            ...
                            <small class="fs-13"><i
                                    class="feather feather-clock text-muted me-1"></i>{{ lang('Created Date') }} <span
                                    class="text-muted">{{ \Carbon\Carbon::parse($tickettrashedview->created_at)->diffForHumans() }}</span></small>
                        </div>
                            ...
                            <div>
                                ...
                                <div class="row galleryopen">
                                    @foreach ($tickettrashedview->getMedia('ticket') as $tickettrashedviewss)
                                        <div class="file-image-1 removespruko{{ $tickettrashedviewss->id }}"
                                            id="imageremove{{ $tickettrashedviewss->id }}">
                                            <div class="product-image">
                                                <a href="{{ $tickettrashedviewss->getFullUrl() }}" class="imageopen">
                                                    <img src="{{ $tickettrashedviewss->getFullUrl() }}" class="br-5"
                                                        alt="{{ $tickettrashedviewss->file_name }}">
                                                </a>
                                        ...
                                        <div class="card-body">
                                            <div class="d-sm-flex">
                                                <div class="d-flex me-3">
                                                    <a href="#">
                                                        @if ($comment->user != null)
                                                            ...
                                                            @else
                                                                <img class="media-object brround avatar-lg"
                                                                    alt="{{ $comment->user->image }}"
                                                                    src="{{ route('getprofile.url', ['imagePath' =>$comment->user->image,'storage_disk'=>$comment->user->storage_disk ?? 'public']) }}">
                                                            @endif
                                                        @else
                                                            <img src="{{ asset('uploads/profile/user-profile.png') }}"
                                                                class="media-object brround avatar-lg" alt="default">
                                                ...
                                                <div class="media-body">
                                                    @if ($comment->user != null)
                                                        <h5 class="mt-1 mb-1 font-weight-semibold">
                                                            {{ $comment->user->name }}@if (!empty($comment->user->getRoleNames()[0]))
                                                                <span
                                                                    class="badge badge-primary-light badge-md ms-2">{{ $comment->user->getRoleNames()[0] }}</span>
                                                            @endif
                                                        </h5>
                                                    @else
                                                    ...
                                                    <div class="editsupportnote-icon animated"
                                                        ...
                                                        <form action="{{ url('admin/ticket/editcomment/' . $comment->id) }}"
                                                            method="POST">
                                                            @csrf
                                                    ...
                                                    @if (Auth::id() == $comment->user_id)
                                                        <div class="row galleryopen">
                                                            @foreach ($comment->getMedia('comments') as $commentss)
                                                                <div class="file-image-1  removespruko{{ $commentss->id }}"
                                                                    id="imageremove{{ $commentss->id }}">
                                                                    <div class="product-image  ">
                                                                        <a href="{{ $commentss->getFullUrl() }}"
                                                                            class="imageopen">
                                                                            <img src="{{ $commentss->getFullUrl() }}"
                                                                                class="br-5"
                                                                                alt="{{ $commentss->file_name }}">
                                                                        </a>
                                                                    </div>
                                        ...
                                        <div class="d-sm-flex">
                                            <div class="d-flex me-3">
                                                <a href="#">
                                                    ...
                                                    @else
                                                        <img class="media-object brround avatar-lg"
                                                            alt="{{ $comment->cust->image }}"
                                                            src="{{ route('getprofile.url', ['imagePath' =>$comment->cust->image,'storage_disk'=>$comment->cust->storage_disk ?? 'public']) }}">
                                                        {{-- <img class="media-object brround avatar-lg"
                                                            alt="{{ $comment->cust->image }}"
                                                            src="@if (Str::contains($comment->cust->image, 'storj')) {{ route('profile.url', ['imagePath' => $comment->cust->image]) }} @else{{ asset('uploads/profile/' . $comment->cust->image) }} @endif"> --}}
                                                    @endif
                                            ...
                                            <div class="media-body">
                                                <h5 class="mt-1 mb-1 font-weight-semibold">
                                                    {{ $comment->cust->username }}<span
                                                        class="badge badge-danger-light badge-md ms-2">{{ $comment->cust->userType }}</span>
                                                </h5>
                                                <small class="text-muted"><i class="feather feather-clock"></i>
                                                    {{ \Carbon\Carbon::parse($comment->created_at)->diffForHumans() }}</small>
                                                <div class="fs-13 mb-0 mt-1">
                                                        ...
                                                        <div class="file-image-1  removespruko{{ $commentss->id }}"
                                                            ...
                                                            <div class="product-image  ">
                                                                <a href="{{ $commentss->getFullUrl() }}"
                                                                    class="imageopen">
                                                                    <img src="{{ $commentss->getFullUrl() }}"
                                                                        class="br-5" alt="{{ $commentss->file_name }}">
                                                                </a>
                                                            </div>
                                                            <span class="file-name-1">
                                                                {{ Str::limit($commentss->file_name, 10, $end = '.......') }}
                                ...
                                <table class="table mb-0 table-information">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="font-weight-semibold">#{{ $tickettrashedview->ticket_id }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                @if ($tickettrashedview->category_id != null)
                                                    @if ($tickettrashedview->category != null)
                                                        <span
                                                            class="font-weight-semibold">{{ $tickettrashedview->category->name }}</span>
                                                    @else
                                                        ~
                                                    @endif
                                                @else
                                        ...
                                        @if ($tickettrashedview->subcategory != null)
                                            <tr>
                                                ...
                                                <td>
                                                    <span
                                                        class="font-weight-semibold">{{ $tickettrashedview->subcategoriess->subcategoryname }}</span>

                                                </td>
                                            </tr>
                                        @endif

                                        @if ($tickettrashedview->item_name != null)
                                        <tr>
                                            <td>
                                            ...
                                            <td>:</td>
                                            <td>
                                                <span class="font-weight-semibold">{{$tickettrashedview->item_name}}</span>
                                        ...
                                        @if ($tickettrashedview->project != null)
                                            <tr>
                                                ...
                                                <td>
                                                    <span
                                                        class="font-weight-semibold">{{ $tickettrashedview->project }}</span>
                                                </td>
                                            </tr>
                                        @endif
                                            ...
                                            <tr>
                                                ...
                                                <td id="priorityid">
                                                    @if ($tickettrashedview->priority == 'Low')
                                                        <span
                                                            class="badge badge-success-light">{{ $tickettrashedview->priority }}</span>
                                                    @elseif($tickettrashedview->priority == 'High')
                                                        <span
                                                            class="badge badge-danger-light">{{ $tickettrashedview->priority }}</span>
                                                    @elseif($tickettrashedview->priority == 'Critical')
                                                        <span
                                                            class="badge badge-danger-dark">{{ $tickettrashedview->priority }}</span>
                                                    @else
                                                        <span
                                                            class="badge badge-warning-light">{{ $tickettrashedview->priority }}</span>
                                                    @endif
                                                </td>
                                            </tr>
                                        @else
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="font-weight-semibold">{{ \Carbon\Carbon::parse($tickettrashedview->created_at)->timezone(Auth::user()->timezone)->format(setting('date_format')) }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                @if ($tickettrashedview->status == 'New')
                                                    <span
                                                        class="badge badge-burnt-orange">{{ $tickettrashedview->status }}</span>
                                                @elseif($tickettrashedview->status == 'Re-Open')
                                                    <span class="badge badge-teal">{{ $tickettrashedview->status }}</span>
                                                @elseif($tickettrashedview->status == 'Inprogress')
                                                    <span
                                                        class="badge badge-info">{{ $tickettrashedview->status }}</span>
                                                @elseif($tickettrashedview->status == 'On-Hold')
                                                    <span
                                                        class="badge badge-warning">{{ $tickettrashedview->status }}</span>
                                                @else
                                                    <span
                                                        class="badge badge-danger">{{ $tickettrashedview->status }}</span>
                                                @endif
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($tickettrashedview->replystatus == 'Solved')
                                                        <span
                                                            class="badge badge-success">{{ $tickettrashedview->replystatus }}</span>
                                                    @elseif($tickettrashedview->replystatus == 'Unanswered')
                                                        <span
                                                            class="badge badge-danger-light">{{ $tickettrashedview->replystatus }}</span>
                                                    @elseif($tickettrashedview->replystatus == 'Waiting for response')
                                                        <span
                                                            class="badge badge-warning">{{ $tickettrashedview->replystatus }}</span>
                                                    @else
                                                    @endif
                            ...
                            <div class="profile-pic">
                                <div class="profile-pic-img mb-2">
                                    <span class="bg-success dots" data-bs-toggle="tooltip" data-bs-placement="top"
                                        title="{{ lang('Online') }}"></span>
                                    @if ($tickettrashedview->cust->image == null)
                                        <img src="{{ asset('uploads/profile/user-profile.png') }}"
                                            class="brround avatar-xxl" alt="default">
                                    @else
                                        <img class="brround avatar-xxl" alt="{{ $tickettrashedview->cust->image }}"
                                            src="{{ route('getprofile.url', ['imagePath' =>$tickettrashedview->cust->image,'storage_disk'=>$tickettrashedview->cust->storage_disk ?? 'public']) }}">
                                    @endif
                                ...
                                <table class="table mb-0 table-information">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <span
                                                    class="font-weight-semibold">{{ $tickettrashedview->cust->last_login_ip }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                <span
                                                    class="font-weight-semibold">{{ $tickettrashedview->cust->phone }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                <span
                                                    class="font-weight-semibold">{{ $tickettrashedview->cust->country }}</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                <span
                                                    class="font-weight-semibold">{{ $tickettrashedview->cust->timezone }}</span>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            ...
                            <div class="card-body  item-user">
                                <div id="refresh">
                                    ...
                                    @foreach ($tickettrashedview->ticketnote()->latest()->get() as $note)
                                        <div class="alert alert-light-warning ticketnote"
                                            id="ticketnote_{{ $note->id }}" role="alert">
                                            @if ($note->user_id == Auth::id())
                                            @endif

                                            <p class="m-0">{{ $note->ticketnotes }}</p>
                                            <p class="text-end mb-0"><small><i><b>{{ $note->users->name }}</b>
                                                        @if (!empty($note->users->getRoleNames()[0]))
                                                            ({{ $note->users->getRoleNames()[0] }})
                                                        @endif
                                                    </i></small></p>
                                        </div>
                                    @endforeach
                                ...
                                <div class="text-center ">
                                    <div class="avatar avatar-xxl empty-block mb-4">
                                        <svg xmlns="http://www.w3.org/2000/svg" height="50" width="50"
                                            ...
                                            <path fill="#CDD6E0" d="M11 8.9c0 1.1-.9 2-2 2H.1L11 0v8.9z" />
                                            <path fill="#FFD05C" d="M15.5 8.6h13.8v2.5H15.5z" />
                                            <path fill="#dbe0ef"
                                                d="M6.3 31.4h9.8v2.5H6.3zM6.3 23.8h22.9v2.5H6.3zM6.3 16.2h22.9v2.5H6.3z" />
                                            <path fill="#FFD15C" d="M22.8 35.7l-2.6 6.4 6.4-2.6z" />
                                            <path fill="#334A5E" d="M21.4 39l-1.2 3.1 3.1-1.2z" />
                                            <path fill="#FF7058" d="M30.1 18h5.5v23h-5.5z"
                                                transform="rotate(-134.999 32.833 29.482)" />
                                            <path fill="#40596B"
                                                d="M46.2 15l1 1c.8.8.8 2 0 2.8l-2.7 2.7-3.9-3.9 2.7-2.7c.9-.6 2.2-.6 2.9.1z" />
                                            <path fill="#F2F2F2" d="M39.1 19.3h5.4v2.4h-5.4z"
                                                transform="rotate(-134.999 41.778 20.536)" />
                                        </svg>
                                    </div>
                ...
                $('body').on('click', '#purchasecodebtn', function() {
                    ...
                    @else
                        ...
                        @else
                            ...
                            var envatopurchase_i = new Array(envatopurchase_id.length - trailingCharsIntactCount +
                                1).join('*') + envatopurchase_id.slice(-trailingCharsIntactCount);
                        @endif
                    @endif
