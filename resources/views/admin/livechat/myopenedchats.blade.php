...
@section('content')
    ...
    <div class="page-header d-xl-flex d-block">
        <div class="page-leftheader">
            <h4 class="page-title"><span class="font-weight-normal text-muted ms-2">{{lang('My Opened Chats',
                    'menu')}}</span></h4>
        </div>
    </div>
    <!--End Page header-->
            ...
            <div class="col-xl-3">
                ...
                <div class="chat-info border card">
                    <div class="card-header border-0 pb-5">
                        <h4 class="card-title">{{ lang('My Opened Chats') }}</h4>
                    </div>
                    <div>
                                ...
                                @if ($LiveCust && !$LiveCust->lastMessage->delete)
                                    ...
                                    <li class="checkforactive" data-id={{$LiveCust->id}}>
                                        <div class="d-flex align-items-center">
                                            <div class="me-2 lh-1">
                                                <span class="avatar brround" id="new-chat-user-bg" style="background-color: {{ randomColorGenerator(0.7) }}">
                                                    @php
                                            ...
                                            <div class="flex-fill">
                                                <div class="mb-0 d-flex align-items-center justify-content-between">
                                                    ...
                                                    <div class="dropdown chat-actions lh-1">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            @if(in_array(Auth::id(), json_decode($LiveCust->mark_as_unread ?? '[]')))
                                                                <li><a href="{{route('admin.removeUserFromUnread')}}?id={{$LiveCust->id}}" class="dropdown-item markAsreadBtn" href="javascript:void(0);"><i
                                                                    class="ri-mail-check-fill align-middle me-2 fs-17 text-muted chat-info-optionicon"></i>{{ lang('Mark
                                                                As Read') }}</a></li>
                                                            @else
                                                                <li><a href="{{route('admin.conversationUnread')}}?id={{$LiveCust->id}}" class="dropdown-item markAsUnreadBtn" href="javascript:void(0);"><i
                                                                    class="ri-mail-unread-fill align-middle me-2 fs-17 text-muted chat-info-optionicon"></i>{{ lang('Mark
                                                                As Unread') }}</a></li>
                                                            @endif
                                                            <li><a class="dropdown-item markassolvedchange" href="javascript:void(0);" data-id="{{$LiveCust->id}}"><i
                                                                        class="ri-mail-check-fill align-middle me-2 fs-17 text-muted chat-info-optionicon"></i>{{ lang('Mark
                                                                    As Solved') }}</a></li>
                                                            ...
                                                            <li>
                                                                <a class="dropdown-item" deleteRouteLink="{{route('admin.livechatConversationDelete')}}?unqid={{$LiveCust->cust_unique_id}}">
                                                                <i class="ri-delete-bin-fill align-middle me-2 fs-17 text-muted chat-info-optionicon"></i>{{ lang('Delete') }}</a>
                                                            </li>
                                                            @if(collect(json_decode($LiveCust->engage_conversation))->count() > 1)
                                                            <li>
                                                                <a class="dropdown-item" href="{{route('admin.conversationLeave')}}?id={{$LiveCust->id}}">
                                                                    <i class="ri-delete-bin-fill align-middle me-2 fs-17 text-muted chat-info-optionicon"></i>
                                                                    {{ lang('Leave') }}
                                                                </a>
                                                            </li>
                                                            @endif
                                                ...
                                                <div class="d-flex align-items-center justify-content-between">
                                                    ...
                                                    @endif
                                                    {{-- Mark As Unread --}}
                                                    @if(in_array(Auth::id(), json_decode($LiveCust->mark_as_unread ?? '[]')) && !$LiveCust->unreadIndexNumber)
                                                        ...
                                                        @foreach (json_decode($LiveCust->engage_conversation) as $key =>$LiveCustmer)
                                                            @if($key < 2)
                                                                @if($LiveCustmer->image)
                                                                    <span class="avatar brround avatar-sm avatar-image-sm"
                        ...
                        <div class="chat-reply-area chat-replyoptions flex-fill textareaDiv d-none">
                            ...
                            <div class="d-flex align-items-center px-2">
                                <div data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Canned responses">
                                    <a aria-label="anchor" class="btn-emoji" href="javascript:void(0)" onclick="{$('#canned-responses').modal('show')}">
                                ...
                                <div class="d-flex align-items-center ms-auto gap-2">
                                    ...
                                    @if(setting('liveChatAgentFileUpload') == '1' || $superadmin->getRoleNames()[0] == 'superadmin')
                                        <a aria-label="anchor" class="btn-attach"
                                            type="file" onclick="{document.querySelector('#chat-file-upload').click()}"
                                            href="javascript:void(0)" data-bs-toggle="tooltip"
                                            data-bs-placement="top" data-bs-title="Upload"
...
<!-- INTERNAL Web Socket -->
<script domainName='{{url('')}}' wsPort="{{setting('liveChatPort')}}" src="{{ asset('build/assets/plugins/livechat/web-socket.js') }}?v=<?php echo time(); ?>"></script>
...
<script>
    ...
    document.getElementById('cannedResponses').onchange=(ele=>{
        ...
        inputNodeElement.innerHTML =  document.querySelector("#auto-expand").value+ele.target.value
...
<script type="text/javascript">
    ...
    function sideMenuOpenClickFunction(ele){
        ...
        success: function (data) {
            ...
            receiverInfo.className = "d-flex align-items-center chat-conversation-Info border-bottom"
            ...
            localStorage.setItem("livechatMyopenedCustomer",ele.getAttribute("data-id"))
            ...
            //for the chat open right dropdown make solve click event add
            $('.markassolvedchange').on('click', function (ele){
                var selectedValue = $(this).data("id");
                $(ele.target).closest('.dropdown-menu').removeClass('show');
                let data = {
                    id : selectedValue,
                    solvedsourcefrom : 'directsolved',
                }
                $.ajax({
                    type: "get",
                    url: '{{route('admin.markAsSolved')}}',
                    data: data,
                    success: function (data) {
                        if(localStorage.getItem("livechatMyopenedCustomer")){
                            localStorage.removeItem("livechatMyopenedCustomer");
                        }
                        toastr.success('This chat is moved to Solved chats.');
                        location.reload();
                    },
                    error: function (data) {
                        console.log('Error:', data);
    ...
    document.querySelectorAll(".checkforactive").forEach((ele)=>{
        ele.onclick = ()=>{
            ...
            if(ele.getAttribute("data-id") == newMessageSoundCurrentAudio && newMessageSoundCurrentAudio.cusumerId && !ele.classList.contains('active')){
    ...
    document.querySelectorAll('#chat-msg-scroll .chat-actions').forEach((ele)=>{
            ...
            if(ele.querySelector('.markAsUnreadBtn')){
                ele.querySelector('.markAsUnreadBtn').addEventListener('click', function (event) {
                    event.stopPropagation();
                    if(localStorage.livechatMyopenedCustomer == ele.closest('.checkforactive').getAttribute('data-group-uniq') || localStorage.livechatMyopenedCustomer == ele.closest('.checkforactive').getAttribute('data-id')){
                        localStorage.removeItem('livechatMyopenedCustomer')
                    }
                })
            }
    ...
    let myopenedcustVal = localStorage.livechatMyopenedCustomer ?? document.querySelector("#chat-msg-scroll li")?.getAttribute("data-id");
    if(myopenedcustVal){
        if(document.querySelector(`.checkforactive[data-id="${myopenedcustVal}"]`)){
            sideMenuOpenClickFunction(document.querySelector(`.checkforactive[data-id="${myopenedcustVal}"]`))
        }else{
    ...
    document.querySelector("#auto-expand").oninput = (ele)=>{
            ...
            // To Remove the Disabled For The Ticket Create Btn
            if( document.querySelector("#livechatTicketSubject").value &&
                ele.target.value &&
                document.querySelector("#category").value &&
                envatoId
                ){
    ...
    Echo.channel('liveChat').listen('ChatMessageEvent',(socket)=>{
        ...
        // To new reAssign user add reload page
        if(socket.comments && socket.comments.includes(`Reassign discussion to ${autoUser}`) && !document.querySelector(`.list-unstyled [data-id='${socket.customerId}']`)){
            location.reload()
        }
        ...
        // To remove the text Input For the Auto Close
        if(socket.comments == "The customer did not provide a response, and as a result, the discussion was automatically marked as solved at" && document.querySelector(`#operator-conversation[operator-id='${socket.customerId}']`)){
            document.querySelector(".chat-footer").classList.add("d-none")
        }
        ...
        if(socket.messageType != "feedBack" && (socket.customerId == liveChatConversation?.getAttribute("operator-id") || socket.id == liveChatConversation?.getAttribute("operator-id")) && socket.message && socket.message != "newUser" && foundUser(socket.id,socket.userName)){
            ...
            // To Open the Image Viewer
            if(custMessage.querySelector(".imageMessageLiveChat")){
                custMessage.querySelector(".imageMessageLiveChat").onclick = (ele)=>{
                    document.querySelector(".liveChatImageViewer").classList.remove("d-none")
                    document.querySelector(".liveChatImageViewer img").src = ele.target.getAttribute("imagesrc")
                    document.querySelector(".liveChatImageViewer .liveChatImageClose").onclick = ()=>{
                        // To Close the Image Viewer
        ...
        if(socket.typingMessage && liveChatConversation?.getAttribute("operator-id") == socket.customerId && !socket.agentInfo){
            ...
            custMessage.setAttribute('typingUserId',socket.customerId)
            custMessage.innerHTML = `
                <div class="chat-list-inner">
                    <div class="chat-user-profile">
                        <span class="avatar avatar-md brround" style="${document.querySelector(`.checkforactive[data-id='${socket.customerId}'] #new-chat-user-bg`)?.getAttribute('style')}">
        ...
        if(socket.typingMessage && liveChatConversation?.getAttribute("operator-id") == socket.customerId && socket.agentInfo && socket.id != autoID){
            ...
            agentMessage.setAttribute('typingUserId',socket.id)
        ...
        if(document.querySelector(`.checkforactive[data-id='${socket.customerId}']`) && socket.typingMessage  && socket.id != autoID){
            if(!document.querySelector(`.checkforactive[data-id='${socket.customerId}'] .custrecentmessage`).innerText.includes('Typing ...')){
                // To add the Old Trying message to the sidebar
                if(pastMessage?.customerId){
                    document.querySelector(`.checkforactive[data-id='${pastMessage?.customerId}'] .custrecentmessage`).innerText = pastMessage
                }
                pastMessage.__proto__.customerId = socket.customerId
