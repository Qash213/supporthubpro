    ...
    <tbody id="refresh">
        ...
        @foreach ($alltickets as $tickets)
            <tr {{ $tickets->replystatus == 'Replied' ? 'class=bg-success-transparent' : '' }}
                            ...
                            <ul class="fs-13 font-weight-normal d-flex custom-ul">
                                ...
                                @if ($tickets->purchasecodesupport != null)
                                    @if ($tickets->purchasecodesupport == 'Supported')
                                        <li class="px-2 text-success font-weight-semibold">
                                            {{ lang('Support Active') }}</li>
                                    @endif
                ...
                <td>
                    {{ $tickets->cust->username }} ({{ lang($tickets->cust->userType) }}) @if ($tickets->ticketviolation == 'on')
                        ...
                        @if ($tickets->status == 'Suspend' || $tickets->status == 'Closed')
                            <div class="btn-group">
                                @if ($tickets->ticketassignmutliples->isNotEmpty() && $tickets->selfassignuser_id == null)
                                    ...
                                    <button data-id="{{ $tickets->id }}" class="btn btn-outline-primary"
                                ...
                                @elseif($tickets->ticketassignmutliples->isEmpty() && $tickets->selfassignuser_id != null)
                                    <button class="btn btn-outline-primary dropdown-toggle" data-bs-toggle="dropdown"
                                        disabled>{{ $tickets->selfassign->name }} (self)
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        ...
                                        <li>
                                            <a href="javascript:void(0);" id="selfassigid"
                                                data-id="{{ $tickets->id }}">{{ lang('Self Assign') }}</a>
                                        </li>
                                        <li>
                                            <a href="javascript:void(0)" data-id="{{ $tickets->id }}"
                                                id="assigned">
                                                {{ lang('Other Assign') }}
                                            </a>
                                        </li>
                    ...
                    @if (Auth::user()->can('Ticket Edit'))
                        <a href="{{ url('admin/ticket-view/' . encrypt($tickets->ticket_id)) }}"
                            class="btn btn-sm action-btns edit-testimonial"><i
                                ...
                                data-bs-placement="top" title="{{ lang('Edit') }}"></i></a>
                    @else
                        ~
                    @endif
                    @if (Auth::user()->can('Ticket Delete'))
                        <a href="javascript:void(0)" data-id="{{ $tickets->id }}" class="btn btn-sm action-btns"
                            id="show-delete"><i class="feather feather-trash-2 text-danger"
                                data-id="{{ $tickets->id }}" data-bs-toggle="tooltip" data-bs-placement="top"
...
<script>
    $(function() {
        var prev = {!! json_encode(lang('Previous')) !!};
        var next = {!! json_encode(lang('Next')) !!};
        var nodata = {!! json_encode(lang('No data available in table')) !!};
        var noentries = {!! json_encode(lang('No entries to show')) !!};
        var showing = {!! json_encode(lang('showing page')) !!};
        var ofval = {!! json_encode(lang('of')) !!};
        var maxRecordfilter = {!! json_encode(lang('- filtered from ')) !!};
        var maxRecords = {!! json_encode(lang('records')) !!};
        var entries = {!! json_encode(lang('entries')) !!};
        var show = {!! json_encode(lang('Show')) !!};
        var search = {!! json_encode(lang('Search...')) !!};
        // Datatable
        ...
        $('#customCheckAll').on('click', function() {
            ...
            if ($('.checkall:checked').length == 0)
                document.querySelector('#massdelete').style.display = 'none';
        });
        ...
        $('.checkall').on('click', function() {
            if ($('.checkall:checked').length == 0) {
                ...
                document.querySelector('#massdelete').style.display = 'inline';
            }
