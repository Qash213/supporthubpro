                            ...
                            <div class="login-icons card-body pt-3 pb-0 text-center justify-content-center">
                                @if ($socialAuthSettings->envato_status == 'enable')
                                    <a class="btn header-buttons text-start social-icon-2 btn-lg btn-lime text-white mb-4 btn-block p-0"
                                        ...
                                        title="{{ lang('Login with Envato') }}" onclick="window.location.href = envato;"
                                ...
                                @if ($socialAuthSettings->google_status == 'enable')
                                    <a class="btn header-buttons text-start social-icon-2 btn-lg btn-google text-white mb-4 btn-block p-0"
                                        href="javascript:;" data-bs-toggle="tooltip"
                                        title="{{ lang('Login with Google') }}" onclick="window.location.href = google;"
...
<script type="text/javascript">
    ...
    function login() {
        ...
        if ($('#username').val() == "") {
            $('#err').html('{{ lang('Please enter the email') }}');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response == 1) {
                    ...
                    toastr.error(
                        '{{ lang('The username or password you entered is incorrect. Please Check', 'alerts') }}'
                        );
                    $("#err").hide().html(
                        "{{ lang('The username or password you entered is incorrect. Please Check', 'alerts') }}"
                        ).fadeIn('slow');
                    ...
                    toastr.error(
                        '{{ lang('Your account is currently inactive. Please contact the admin.', 'alerts') }}'
                        );
                } else if (response == 30) {
