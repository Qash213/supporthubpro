        ...
        if ($guest) {
            if ($guest->userType == 'Guest') {
                ...
                try {
                    ...
                    return response()->json(['description' => $ticket->message, 'subject' => $ticket->subject, 'id' => $ticket->id,'guest' => 'pass', 'data' => $ticket, 'success' => lang('A ticket has been opened with the ticket ID', 'alerts') . $ticket->ticket_id], 200);
                }
    ...
    public function postComment(Request $request, $ticket_id)
    {
        ...
        if ($ticket->status == "Closed") {
            ...
            foreach ($request->input('comments', []) as $file) {
                ...
                $provider =  storage()->provider;
                $provider::mediaupload($comment,'uploads/guestticket/' . $file,'comments');
            }
            ...
            if (request()->input(['status']) == 'Closed')
            {
                ...
                if ($ticket->cust->userType == 'Guest') {
                    $ticketData = [
                        ...
                        'ticket_status' => $ticket->status,
                        'comment' => $comment->comment,
                        'ratinglink' => route('guest.rating', encrypt($ticket->ticket_id)),
                        'ticket_customer_url' => route('guest.ticketdetailshow', encrypt($ticket->ticket_id)),
                        'ticket_admin_url' => url('/admin/ticket-view/' . encrypt($ticket->ticket_id)),
                    ];
                }
        ...
        if(setting('RESTRICT_TO_REPLY_TICKET') == 'on' && $commentsNull->all() != null && setting('MAXIMUM_ALLOW_REPLIES') > 0){
            ...
            if(!$totalcount){
                ...
                if (session()->get('sessionverifyemail') == $ticket_id) {
                    return view('guestticket.showticketview', compact('ticket', 'category', 'comments','createdcount'))->with($data);
                } else {
