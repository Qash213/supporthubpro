...
class DashboardController extends Controller
{
   ...
   public function notifystatus(Request $request)
   {
       ...
       if(!$status){
            $notifications = auth()->guard('customer')->user()->notifications()->paginate($perPage, ['*'], 'page', $page)->groupBy(function($date) {
                return \Carbon\Carbon::parse($date->created_at)->format('Y-m-d');
            });
       }
       else{
            $notifications =  auth()->guard('customer')->user()->notifications()->whereIn('data->status', $status)->paginate($perPage, ['*'], 'page', $page)->groupBy(function($date) {
                return \Carbon\Carbon::parse($date->created_at)->format('Y-m-d');
            });
       }
       ...
       $view = view('user.notificationpageinclude',compact('notifications','title', 'footertext', 'seopage', 'status', 'perPage', 'page'))->render();
